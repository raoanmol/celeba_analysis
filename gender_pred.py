...
os.makedirs('./models/gender', exist_ok=True)
...
def main():
    set_seed(42)
    with open('./logs/gender.txt', 'w') as log_file:
        train_paths, validation_paths, test_paths = load_partitioned_data()
        train_labels, validation_labels, test_labels = load_data_annotations(attribute="Male")
        ...
        for epoch in tqdm(range(num_epochs), desc='Epoch Progress', unit='epoch'):
            ...
            if val_acc > best_val_acc:
                ...
                torch.save(model.state_dict(), "./models/gender/gender_accuracy_model.pth")
                best_acc_epoch = epoch + 1

            if val_precision > best_val_precision:
                best_val_precision = val_precision
                torch.save(model.state_dict(), "./models/gender/gender_precision_model.pth")
                best_precision_epoch = epoch + 1

            if val_recall > best_val_recall:
                best_val_recall = val_recall
                torch.save(model.state_dict(), "./models/gender/gender_recall_model.pth")
        ...
        evaluate_model(model, "./models/gender/gender_accuracy_model.pth", "Best Accuracy Model", device, test_loader, log_file)
        evaluate_model(model, "./models/gender/gender_precision_model.pth", "Best Precision Model", device, test_loader, log_file)
        evaluate_model(model, "./models/gender/gender_recall_model.pth", "Best Recall Model", device, test_loader, log_file)

if __name__ == '__main__':
    main()
    run_tcav(target_attr = 'Male', model_path = './models/gender/gender_accuracy_model.pth', target_class = 0, attr_file_path = './data/list_attr_celeba.txt', image_dir = './data/img_align_celeba/', max_images = 100, num_random_set = 2, n_steps = 10, processes = 0, save_plots = True)
