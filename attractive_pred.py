...
os.makedirs('./models/attractiveness', exist_ok=True)
...
def evaluate_model(model, model_path, model_name, device, test_loader, log_file):
    ...
    report = classification_report(
        ...
        target_names=['Not Attractive', 'Attractive'],
        zero_division=0
    )
...
def main():
    set_seed(42)
    with open('./logs/attractiveness.txt', 'w') as log_file:
        train_paths, validation_paths, test_paths = load_partitioned_data()
        train_labels, validation_labels, test_labels = load_data_annotations(attribute="Attractive")
        ...
        for epoch in tqdm(range(num_epochs), desc='Epoch Progress', unit='epoch'):
            ...
            if val_acc > best_val_acc:
                ...
                torch.save(model.state_dict(), "./models/attractiveness/attractive_accuracy_model.pth")
                best_acc_epoch = epoch + 1

            if val_precision > best_val_precision:
                best_val_precision = val_precision
                torch.save(model.state_dict(), "./models/attractiveness/attractive_precision_model.pth")
                best_precision_epoch = epoch + 1

            if val_recall > best_val_recall:
                best_val_recall = val_recall
                torch.save(model.state_dict(), "./models/attractiveness/attractive_recall_model.pth")
        ...
        evaluate_model(model, "./models/attractiveness/attractive_accuracy_model.pth", "Best Accuracy Model", device, test_loader, log_file)
        evaluate_model(model, "./models/attractiveness/attractive_precision_model.pth", "Best Precision Model", device, test_loader, log_file)
        evaluate_model(model, "./models/attractiveness/attractive_recall_model.pth", "Best Recall Model", device, test_loader, log_file)

if __name__ == '__main__':
    main()
    run_tcav(target_attr = 'Attractive', model_path = './models/attractiveness/attractive_accuracy_model.pth', target_class = 0, attr_file_path = './data/list_attr_celeba.txt', image_dir = './data/img_align_celeba/', max_images = 100, num_random_set = 2, n_steps = 10, processes = 0, save_plots = True)